...
* Für die Bildskalierung ist der beste und einfachste Ansatz die Verwendung von `&wid=<value>` und `&hei=<value>` oder `&hei=<value>`. Diese Parameter legen die Bildbreite automatisch in der Übereinstimmung mit dem Seitenverhältnis fest.
...
Whitepaper zu Best Practices [Scharfzeichnen von Bildern in Adobe Dynamic Media Classic und auf dem Image-Server](/help/using/assets/s7_sharpening_images.pdf).
...
* `&jpegSize=` interagiert mit dem JPG-Komprimierungsparameter `&qlt=`. Wenn die JPG-Antwort mit dem angegebenen JPG-Komprimierungsparameter ( `&qlt=`) überschreitet nicht die `jpegSize` -Wert, wird das Bild mit `&qlt=` wie definiert. Andernfalls `&qlt=` wird schrittweise reduziert, bis das Bild der maximal zulässigen Größe entspricht oder bis das System bestimmt, dass es nicht passt, und einen Fehler zurückgibt.

Als Best Practice gilt Folgendes: `&jpegSize=` und den Parameter hinzufügen `&qlt=` wenn Sie JPG-Bilder an Geräte mit begrenztem Speicher bereitstellen.
